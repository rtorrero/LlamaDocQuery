...
The Hub prepares and provides content for multiple peripheral Uyuni Servers.
...
The main database is a PostgreSQL database in the Uyuni Hub system.
...
The whole Uyuni Hub environment should be using only one Root CA.
====
...
Every table gets an extra id column (`mgm_id`) specifying the Uyuni server which provided the data.
On a single Uyuni Server this column has the standard value `1` which represent [literal]``localhost``.
On the Uyuni Hub it is replaced with the real server id the managed server has in the hub database.
...
On the Uyuni Hub there is a second schedule which is important.
